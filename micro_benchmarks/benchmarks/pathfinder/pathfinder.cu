...
static unsigned long long rows = 8188;
static unsigned long long cols = 262144;
int* data;
int* gpuWall;
int* gpuResult[2];
#define M_SEED 9
static int pyramid_height = 10;
...
parse_args(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "r:c:p:eh")) != -1) {
        switch (c) {
            case 'r':
                rows = parse_count(optarg, "rows");
                break;
            case 'c':
                cols = parse_count(optarg, "cols");
                break;
            case 'p':
                pyramid_height = parse_count(optarg, "pyramid_height");
                break;
            case 'e':
